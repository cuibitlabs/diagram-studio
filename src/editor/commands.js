...
export function buildCommands(actions, state) {
  ...
  const commands = [
    ...
    { id: "simplify-light", group: "Layout", label: "Simplify: light (merge duplicates, fold parallel links)", run: () => actions.simplify("light") },
    { id: "simplify-balanced", group: "Layout", label: "Simplify: balanced (also drop unconnected decoration)", run: () => actions.simplify("balanced") },
    { id: "simplify-aggressive", group: "Layout", label: "Simplify: aggressive (also collapse pass-through nodes)", run: () => actions.simplify("aggressive") },
